...
import { SlideOver, AddBrandForm, AddLocationForm } from './AdminActions'
...
export default function ProfileMenu({ role, variant }: { role: Role; variant?: 'users' }) {
  ...
  return (
    <>
        ...
        {open && (
          <div className={styles.menu}>
            ...
            {role === 'admin' && !isUsersPage && (
              <>
                <button className={styles.menuLink} type="button" onClick={() => openModal('brand')}>
                  {PLUS}
                  Add Brand
                </button>
                <button className={styles.menuLink} type="button" onClick={() => openModal('location')}>
                  {PLUS}
                  Add Location
                </button>
              </>
      ...
      {modal === 'brand' && (
        <SlideOver title="Add Brand" onClose={() => setModal(null)}>
          <AddBrandForm onSuccess={handleModalSuccess} />
        </SlideOver>
      )}
      {modal === 'location' && (
        <SlideOver title="Add Location" onClose={() => setModal(null)}>
          <AddLocationForm onSuccess={handleModalSuccess} />
        </SlideOver>
      )}
    </>
  )
