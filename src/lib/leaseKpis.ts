...
export type KpiTile = { label: string; value: string; sub?: string }

// KPI tiles use '--' (not the '—' fmtDate defaults to) to match this grid's existing look.
const DASH = '--'

export function buildKpiTiles(lease: LeaseWithRelations): KpiTile[] {
  const now = new Date()
  ...
  const { monthly: currentMonthly, annual: currentAnnual } = getCurrentRentPeriod(lease)
  ...
  return [
    ...
    {
      label: 'Commencement',
      value: fmtDate(lease.commencement_date, DASH),
      sub: 'Lease Start',
    },
    {
      label: 'Expiry',
      value: fmtDate(lease.expiry_date, DASH),
    ...
    },
    {
      ...
      value: fmtDate(lease.execution_date, DASH),
      sub: 'Date Signed',
    },
  ]
}
